...
## Internal modules.
from setup_losses import get_base
...
def get_eval(loss_name=None, model_name=None, **kwargs):
    ...
    ## Always evaluate using at least the base loss.
    ## Note: the spectral loss evaluator is added in driver script.
    loss = get_base(name=loss_name)
    ...
    if loss_name == "logistic":
        loss_01 = get_base(name="zero_one")
...
def eval_model(epoch, model, storage, data, eval_dict):
    ...
    for key in store_test.keys():
        evaluator = eval_dict[key]
        store_test[key][epoch,0] = np.mean(evaluator(model=model,
                                                     X=X_test,
                                                     y=y_test))
    return None
